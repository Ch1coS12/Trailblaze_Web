...
public final class WorkSheetUtil {
    ...
    public static ObjectNode toDetailJson(Entity ws,
        ...
        ops.forEachRemaining(op -> {
            ...
            // Add operation ID (either from key name or key id)
            String opId = op.getKey().getName();
            if (opId == null) {
                opId = String.valueOf(op.getKey().getId());
            }
            o.put("id", opId);
        ...
        parcels.forEachRemaining(p -> {
            try {
                ...
                // Add parcel ID (either from key name or key id)
                String parcelId = p.getKey().getName();
                if (parcelId == null) {
                    parcelId = String.valueOf(p.getKey().getId());
                }
                pr.put("id", parcelId);
                ...
                if (p.contains("areaHa")) {
                    pr.put("area", p.getDouble("areaHa"));
                } else {
                    pr.put("area", 0.0);
                }
