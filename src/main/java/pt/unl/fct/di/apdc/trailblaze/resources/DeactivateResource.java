...
/**
 * POST /rest/deactivate   – Desativa a conta indicada em {@code targetUsername}.
 */
@Path("/deactivate")
@Consumes(MediaType.APPLICATION_JSON)
@Produces(MediaType.APPLICATION_JSON)
public final class DeactivateResource {
    ...
    @POST
    public Response deactivate(AccountStateChangeRequest req,
        ...
        try {
            ...
            /* SYSBO não pode desativar SYSADMIN nem outro SYSBO */
            if (requesterRoles.contains("SYSBO") &&
                (targetRoles.contains("SYSADMIN") || targetRoles.contains("SYSBO")))
                return Response.status(Response.Status.FORBIDDEN)
                               .entity("SYSBO não pode desativar esta conta.")
                               .build();

            if ("DESATIVADA".equals(target.getString("state")))
                return Response.status(Response.Status.CONFLICT)
                               .entity("Conta já está desativada.").build();

            /* -------- persistir mudança -------- */
            DS.put(Entity.newBuilder(target).set("state", "DESATIVADA").build());
            return Response.ok("Conta desativada com sucesso.").build();

        } catch (Exception ex) {                           // token inválido, etc.
            ex.printStackTrace();
            return Response.status(Response.Status.UNAUTHORIZED)
    ...
    /** Extrai lista de roles do JWT. Aceita *claim* nova “roles” ou o campo legado “role”. */
    ...
    /** Extrai lista de roles de uma Entity, compatível com contas antigas. */
